...
class ObliFrame(wx.Frame):
    ...
    def __init__(self, parent, id):  # , stat):
        wx.Frame.__init__(self, parent, id, u'倾角仪实时监控', size=(1083, 720),
...
class ObliPlot():
    ...
    """Paint Obli which contain Force and Temperature
    """
    ...
    def __init__(self, panel, fig):
        ...
        self.oblidata = ObliReceiveThread()
        self.oblidata.start()
    ...
    def InitPlot(self, panel, sub):
        ...
        self.canvas.append(FigCanvas(panel, -1, self.fig))
    ...
    def ChChoose(self, stat):
        ...
        #         np.arange(len(self.data[i])) + 20.0 / 100
        #     )
        #     self.axes[i].set_xticklabels(
        #         ['ch' + str(i) for i in self.stat]
        #     )
    ...
    def ReDraw(self, event):
        ...
        for i in self.stat:
            self.data[0].append(
                self.oblidata.data['ch' + str(i)][0])
            self.data[1].append(
                self.oblidata.data['ch' + str(i)][1])

        self.DrawPlot(0)
        self.DrawPlot(1)


class ObliReceiveThread(threading.Thread):
    """Receive Obli data
