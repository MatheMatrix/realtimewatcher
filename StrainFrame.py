...
class StrainFrame(wx.Frame):
    ...
    def Display(self, chs, gbs, panel):
        ...
        for i in range(1, 9):
            gbs.Add(wx.StaticText(panel, -1, u'通道{0}'.format(i)), (1, 2 * i - 1), flag=wx.ALIGN_CENTER)
            for j in chs['ch' + str(i)]:
                gbs.Add(wx.StaticText(panel, -1, u'传感器{0}'.format(j)),
                        (chs['ch' + str(i)].index(j) + 2, (i - 1) * 2),
                        flag=wx.ALIGN_CENTER)
                self.texts.append(wx.TextCtrl(panel, -1, "0.000", size=(50, -1)))
                gbs.Add(self.texts[-1], (chs['ch' + str(i)].index(j) + 2, 2 * i - 1), flag=wx.ALIGN_CENTER)

        for i in range(9, 17):
            gbs.Add(wx.StaticText(panel, -1, u'通道{0}'.format(i)), (1 + 8, 2 * (i - 8) - 1), flag=wx.ALIGN_CENTER)
            for j in chs['ch' + str(i)]:
                gbs.Add(wx.StaticText(panel, -1, u'传感器{0}'.format(j)),
                        (chs['ch' + str(i)].index(j) + 2 + 8, (i - 8 - 1) * 2),
                        flag=wx.ALIGN_CENTER)
                self.texts.append(wx.TextCtrl(panel, -1, "0.000", size=(50, -1)))
                gbs.Add(self.texts[-1], (chs['ch' + str(i)].index(j) + 2 + 8, 2 * (i - 8) - 1), flag=wx.ALIGN_CENTER)
